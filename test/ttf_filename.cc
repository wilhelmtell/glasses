...
TEST_CASE("default-constructed ttf_filename compiles") {
  gls::ttf_filename t;
}
...
TEST_CASE("8 characters ttf_filename constructed with a C string equals c_str()") {
  gls::ttf_filename const t("01234567");
  REQUIRE(std::string("01234567") == t.c_str());
}

TEST_CASE("1 character ttf_filename constructed with a C string equals c_str()") {
  gls::ttf_filename const t("a");
