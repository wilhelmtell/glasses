...
TEST_CASE("default-constructed bmp_filename compiles") { gls::bmp_filename t; }
...
TEST_CASE(
  "8 characters bmp_filename constructed with a C string equals c_str()") {
  gls::bmp_filename const t("01234567");
  REQUIRE(std::string("01234567") == t.c_str());
}

TEST_CASE(
  "1 character bmp_filename constructed with a C string equals c_str()") {
  gls::bmp_filename const t("a");
