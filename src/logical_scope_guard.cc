...
namespace tls {
logical_scope_guard::logical_scope_guard(cleanup_type cleanup)
: cleanup(std::move(cleanup)) {}

logical_scope_guard::logical_scope_guard(init_type init, cleanup_type cleanup)
: cleanup(std::move(cleanup)) {
  init();
}

logical_scope_guard::logical_scope_guard(logical_scope_guard&& rhs)
...
logical_scope_guard& logical_scope_guard::operator=(
  logical_scope_guard&& rhs) {
  cleanup = std::move(rhs.cleanup);
  rhs.cleanup = nullptr;
  return *this;
}

logical_scope_guard::~logical_scope_guard() {
  if(cleanup) cleanup();
}
}
